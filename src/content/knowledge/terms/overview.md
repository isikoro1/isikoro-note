---
title: "IT用語メモ"
summary: "まだ深掘りしていないIT用語をまとめて置いておくための入口。あとで必要に応じて深掘りノートへ昇格する。"
category: "Note"
type: "index"
status: "draft"
created: "2026-07-17"
updated: "2026-07-22"
tags: ["IT用語", "メモ", "学習"]
aliases: ["用語メモ", "IT用語集", "技術用語メモ"]
keywords:
  - IT用語
  - 技術用語
  - 学習メモ
  - 用語整理
history:
  - date: "2026-07-17"
    text: "初版作成"
  - date: "2026-07-17"
    text: "言語・ライブラリカテゴリ追加に合わせて整理"
  - date: "2026-07-19"
    text: "AI・ChatGPTを追加"
  - date: "2026-07-21"
    text: "logic:equalタグを追加"
  - date: "2026-07-22"
    text: "AWSを追加"
  - date: "2026-07-22"
    text: "プログラミングパラダイム関連ページを追加"
related:
  - title: "索引"
    slug: "reverse/overview"
  - title: "サイト内検索"
    slug: "tech/frontend/site-search"
---

## 概要

IT用語メモは、まだ1ページとして深掘りするほどではない用語をまとめて置いておく場所です。

このノートでは、すべての用語を網羅することは目的にしません。
業務、個人開発、面接、学習の中で出てきた用語を一旦ここに置き、必要になったものだけ深掘りノートとして育てます。

## 使い方

- ざっくり意味を確認する
- 関連する用語を探す
- 後で深掘りする候補を残す
- 面接や業務で出た単語を一時的に置く

深掘りしたくなった用語は、個別ページを厚くするか、深掘りノート側に移します。

## AI

- [AI](/isikoro-note/knowledge/tech/ai/ai/)
- [ChatGPT](/isikoro-note/knowledge/tech/ai/chatgpt/)

## Java周辺・業務システム

- [Java EE](/isikoro-note/knowledge/tech/java/java-ee/)
- [EJB](/isikoro-note/knowledge/tech/java/ejb/)
- [Struts](/isikoro-note/knowledge/tech/java/struts/)
- [Servlet](/isikoro-note/knowledge/tech/java/servlet/)
- [JSP](/isikoro-note/knowledge/tech/java/jsp/)
- [logic:equalタグ](/isikoro-note/knowledge/tech/java/logic-equal/)

## プログラミングの考え方

- [プログラミングパラダイム](/isikoro-note/knowledge/tech/programming-paradigm/overview/)
- [手続き型プログラミング](/isikoro-note/knowledge/tech/programming-paradigm/procedural-programming/)
- [オブジェクト指向](/isikoro-note/knowledge/tech/architecture/object-oriented-programming/)
- [関数型プログラミング](/isikoro-note/knowledge/tech/programming-paradigm/functional-programming/)
- [宣言型プログラミング](/isikoro-note/knowledge/tech/programming-paradigm/declarative-programming/)
- [プログラミングパラダイムの比較](/isikoro-note/knowledge/tech/programming-paradigm/comparison/)

## Web・フロントエンド

- [Cookie](/isikoro-note/knowledge/tech/web/cookie/)
- [Session](/isikoro-note/knowledge/tech/web/session/)
- [JWT](/isikoro-note/knowledge/tech/web/jwt/)
- [REST API](/isikoro-note/knowledge/tech/web/rest-api/)
- [MVC](/isikoro-note/knowledge/tech/architecture/mvc/)

## データベース

- [Transaction](/isikoro-note/knowledge/tech/database/transaction/)
- [コネクションプール](/isikoro-note/knowledge/tech/database/connection-pool/)
- [Deadlock](/isikoro-note/knowledge/tech/database/deadlock/)
- [排他制御](/isikoro-note/knowledge/tech/database/exclusive-control/)
- [分離レベル](/isikoro-note/knowledge/tech/database/isolation-level/)
- [N+1問題](/isikoro-note/knowledge/tech/database/n-plus-one/)
- [DBインデックス](/isikoro-note/knowledge/tech/database/db-index/)

## クラウド・インフラ

- [AWS](/isikoro-note/knowledge/tech/cloud/aws/)

## ネットワーク

- [TCP/IP](/isikoro-note/knowledge/tech/network/tcp-ip/)
- [OSI参照モデル](/isikoro-note/knowledge/tech/network/osi-reference-model/)

## 設計・障害対応

- [冪等性](/isikoro-note/knowledge/tech/architecture/idempotency/)
- [リトライ](/isikoro-note/knowledge/tech/architecture/retry/)
- [タイムアウト](/isikoro-note/knowledge/tech/architecture/timeout/)

## 業務・金融

- [金融ドメイン](/isikoro-note/knowledge/work/finance/financial-domain/)
- [クレジットカードシステム](/isikoro-note/knowledge/work/finance/credit-card-system/)
- [オーソリ](/isikoro-note/knowledge/work/finance/authorization/)
- [売上確定](/isikoro-note/knowledge/work/finance/capture/)
- [精算](/isikoro-note/knowledge/work/finance/settlement/)

## 運用方針

浅い用語メモは、このページから辿れる状態にしておけば十分です。
用語そのものを増やすより、実装したもの、判断したもの、詰まったものを深掘りしていきます。
